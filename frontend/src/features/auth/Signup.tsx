        ...
        <div className="min-h-screen w-full flex bg-[#030712] text-white font-mono selection:bg-brand-500/30 overflow-hidden">
                ...
                <div className="max-w-md w-full mx-auto">
                    ...
                    <div className="mb-10">
                         <div className="flex items-center gap-2 text-brand-400 mb-4 animate-pulse">
                            ...
                            <span className="text-xs tracking-widest uppercase">New User Protocol</span>
                        </div>
                        <h1 className="text-4xl md:text-5xl font-bold tracking-tighter text-white mb-2">
                             <span className="text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400">Initialize.</span>
                            <br />
                            <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-400 to-purple-400">Join Network.</span>
                        </h1>
                    </div>
                    ...
                    <form className="space-y-6">
                         <div className="space-y-2 group">
                            <label className="text-xs uppercase tracking-widest text-slate-500 group-focus-within:text-brand-400 transition-colors">Client.FullName</label>
                        ...
                        <div className="space-y-2 group">
                            <label className="text-xs uppercase tracking-widest text-slate-500 group-focus-within:text-brand-400 transition-colors">Client.Email</label>
                            <div className="relative">
                                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-600 select-none">{'>'}</span>
                                <input 
                                    type="email" 
                        ...
                        <div className="space-y-2 group">
                            <label className="text-xs uppercase tracking-widest text-slate-500 group-focus-within:text-brand-400 transition-colors">Secure.Password</label>
                            <div className="relative">
                                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-600 select-none">{'>'}</span>
                                <input 
                                    type="password" 
                                    className="w-full bg-slate-900/50 border border-slate-800 focus:border-brand-500/50 rounded-none px-4 py-4 pl-8 text-white outline-none transition-all placeholder:text-slate-700 font-mono focus:bg-slate-900"
                                    placeholder="create_passcode..."
                                />
                                <div className="absolute top-0 right-0 h-full w-1 bg-brand-500 opacity-0 group-focus-within:opacity-100 transition-opacity"></div>
                            </div>
                        </div>
                    ...
                    <div className="mt-12 pt-6 border-t border-slate-900 flex justify-between items-center text-xs text-slate-500">
                        <p>NETWORK ACCESS v2.1</p>
                        <button onClick={() => navigate('/login')} className="text-brand-400 hover:text-brand-300 hover:underline tracking-wider uppercase">
                            [Access_Login]
                        </button>
                    </div>
                </div>
            ...
            {/* RIGHT SIDE: AI Visual Core */}
             <div className="hidden lg:flex w-1/2 relative bg-black items-center justify-center p-12 overflow-hidden border-l border-slate-900">
                ...
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] bg-cyan-500/10 rounded-full blur-2xl animate-pulse delay-75"></div>

                {/* Central AI Visual */}
                <div className="relative z-10 w-full max-w-sm">
                    <div className="relative border border-slate-800 bg-black/50 backdrop-blur-sm p-6 rounded-lg animate-in fade-in zoom-in-95 duration-1000 border-r-4 border-r-brand-500 text-right">
                         <div className="flex justify-between items-center mb-4 border-b border-slate-800 pb-2">
                             <div className="text-[10px] text-slate-600 font-mono">NODE: NEW_USER_01</div>
                            <div className="text-xs text-brand-500 font-mono flex items-center gap-2">
                                AI_REGISTRATION
                                <span className="relative inline-flex rounded-full h-2 w-2 bg-brand-500"></span>
                            </div>
                        </div>
                        <div className="space-y-2 font-mono text-sm leading-relaxed" dir="rtl">
                            <p className="text-slate-400">Verifying identity protocols <span className="text-cyan-500">➜</span></p>
                            <p className="text-slate-400">Allocating server resources <span className="text-cyan-500">➜</span></p>
                            <p className="text-slate-400">Preparing workspace <span className="text-cyan-500">➜</span></p>
                            <div className="mt-4 p-3 bg-slate-900/50 border border-slate-800 text-xs text-slate-500 text-left" dir="ltr">
                                {`{ "step": "init", "user_type": "dev", "mode": "accelerated" }`}
                            </div>
                        </div>
                    </div>

                     {/* Floating Nodes Decoration */}
                     <div className="absolute bottom-[-50px] left-[-50px] w-32 h-32 border border-brand-500/20 rounded-full flex items-center justify-center animate-[spin_15s_linear_infinite_reverse]">
                        <div className="w-1.5 h-1.5 bg-cyan-500 rounded-full mb-10"></div>
                     </div>
                </div>
            </div>
        </div>
